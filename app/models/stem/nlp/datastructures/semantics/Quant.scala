...
//  val geen: Pred => Entity = p => {
//    val negatedEntities= for (e <- p.entities) yield {
//      "n"+e
//    }
//    val disjunction: Set[Set[String]]=Set(negatedEntities)
...
//  val meeste: Pred => Entity = p => {
//    val powerset:Set[Set[String]]=p.entities.subsets.map(_.toSet).toSet
//    val disjunction:Set[Set[String]]=powerset.filter(_.size>(p.entities.size/2))
//    val e: Entity = Entity(disjunction)
//    e
//  }
}
